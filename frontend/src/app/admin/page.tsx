...
export default function AdminPage() {
  ...
  const [monitorResult, setMonitorResult] =
    useState<TriggerMonitorRunResponse | null>(null);
  ...
  const totalPayouts = allClaims.reduce(
    (sum, claim) =>
      sum + (claim.payout_status === "processed" ? claim.payout_amount : 0),
    0,
  );
  const claimsByType = summary?.claims_by_event_type
    ? Object.entries(summary.claims_by_event_type).map(
        ([eventType, count]) => ({
    ...
    : [];
  const claimsByStatus = summary?.claims_by_status
          ...
          <>
            {monitorResult && (
              <Card padding="md" className="mb-6">
                <CardHeader
                  title="Latest Monitor Run"
                  action={
                    <Badge variant="info">{monitorResult.monitor_run_id}</Badge>
                  }
                />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
              <div className="lg:col-span-2">
                <Card padding="lg">
                  <CardHeader
                    ...
                    subtitle="Live claim counts by backend event type"
                  />
                  <div className="h-64">
              ...
              <Card padding="lg">
                <CardHeader
                  ...
                  subtitle="Current claim pipeline mix"
                />
                <div className="h-48">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              <Card padding="lg">
                <CardHeader
                  title="Recent Events"
                  subtitle="Latest backend event records"
                />
                    ...
                    summary.recent_events.map((event) => (
                      ...
                      >
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            <p className="text-[13px] font-medium text-text-primary">
                              {getEventInfo(event.event_type).label}
                            </p>
                        ...
                        <p className="text-[11px] text-text-muted mt-2">
                          ...
                          {formatNumber(event.duration_hours, 1)}h •{" "}
                          {event.source}
                        </p>
                      </div>
                    ))
              ...
              <Card padding="lg">
                <CardHeader
                  title="Recent Claims"
                  subtitle="Latest auto-created claim records"
                />
                <div className="space-y-3">
                      ...
                      >
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            ...
                            <p className="text-[12px] text-text-secondary">
                              {getEventInfo(claim.event_type).label}
                            </p>
                          </div>
                          <Badge
            ...
            <Card padding="lg" className="mb-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                {summary.forecast_cards.map((card, index) => (
                  <div
                    key={`${card.city}-${card.zone}-${card.shift_type}-${index}`}
                    className="p-3 rounded-lg border border-border bg-background"
                  >
                    <div className="flex items-center justify-between gap-3 mb-2">
                      ...
                      <Badge variant={getRiskBadgeVariant(card.risk_band)}>
                        {card.risk_band}
                      </Badge>
                    </div>
                    <p className="text-[12px] text-text-secondary">
                      {card.city} • {getShiftLabel(card.shift_type)}
                    </p>
                    <div className="mt-3 space-y-1 text-[12px]">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium text-text-primary">
                          {formatNumber(card.expected_disrupted_hours)}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-text-secondary">
                        ...
                        <span className="font-medium text-text-primary">
                          {formatCurrencyValue(card.suggested_weekly_premium)}
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="mt-4 px-3 py-2.5 rounded-lg bg-border-light flex items-start gap-2">
                ...
                <p className="text-[12px] text-text-secondary">
                  Forecast cards are useful for demo dashboards. Claim approval
                  still stays separate from ML and remains rule-based.
                </p>
              </div>
            </Card>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-[12px] text-text-secondary mt-0.5">
                    Total live payouts so far:{" "}
                    {formatCurrencyValue(totalPayouts)} across{" "}
                    {allClaims.length} claim records.
                  </p>
                </div>
              </div>
            </Card>
