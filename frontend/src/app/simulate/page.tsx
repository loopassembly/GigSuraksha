...
export default function SimulatePage() {
      ...
      try {
        ...
        if (!storedPolicy || storedPolicy.status !== "active") {
          if (requestError instanceof ApiError) {
            ...
            setContextError(
              "Unable to load the active policy context right now.",
            );
          }
        }
      } finally {
  ...
  const insuredShiftWindow = getShiftWindowFromBackend(
    policy?.shift_type ?? worker?.shift_type,
  );
  ...
  const activeResult =
    result &&
    activePolicy &&
    result.event.city === activePolicy.city &&
    result.event.zone === activePolicy.zone
      ? result
      : null;
  ...
  async function handleTrigger() {
    if (!activePolicy) {
      setSimulationError(
        "Activate a policy before running a worker-bound disruption simulation.",
      );
      return;
    }
    ...
    if (!liveEventType) {
      setSimulationError(
        "This disruption type is not supported by the live simulation API.",
      );
      return;
    }
    ...
    try {
      ...
      if (requestError instanceof ApiError) {
        ...
        setSimulationError(
          "Unable to trigger the live simulation right now. Please try again.",
        );
      }
    } finally {
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-[14px] text-text-secondary mt-1">
                  Worker-side disruption simulation is tied to the active
                  insured zone and shift stored in the backend. Create a worker
                  profile and activate weekly protection before simulating a
                  disruption.
                </p>
                {contextError && (
    ...
    <>
      ...
      <main className="flex-1 max-w-5xl mx-auto px-4 py-6 sm:py-10 w-full">
        <div className="mb-6">
          ...
          <p className="text-[14px] text-text-secondary mt-1">
            Simulate what happens if a verified disruption affects this worker’s
            active insured zone and shift.
          </p>
        </div>
          ...
          <div className="lg:col-span-3 space-y-5">
            <Card padding="lg">
              <CardHeader
                ...
                subtitle="Adjust the disruption event only. Policy context comes from the active backend policy."
              />
              ...
              <div className="mb-5 p-4 rounded-lg border border-border bg-background">
                <div className="flex items-center justify-between gap-3 mb-3">
                  <div>
                    ...
                    <p className="text-[12px] text-text-secondary mt-0.5">
                      Simulations are evaluated against the worker&apos;s active
                      insured shift stored in the backend.
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[13px]">
                  ...
                  <ContextRow
                    label="Insured Shift"
                    value={getShiftLabel(activePolicy.shift_type)}
                  />
                  <ContextRow
                    label="Coverage Tier"
                    value={getCoverageTierLabel(activePolicy.coverage_tier)}
                  />
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {DISRUPTION_TYPES.map((disruptionType) => {
                    ...
                    const DisruptionIcon =
                      iconMap[disruptionInfo.icon] || CloudRain;
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          <span
                            className={`font-medium ${type === disruptionType ? "text-primary" : "text-text-secondary"}`}
                          >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setEventStartHour(Number(event.target.value))
                    }
                    className="w-full px-3 py-2 border border-border rounded-lg text-[13px] text-text-primary bg-surface outline-none focus:border-primary"
                  >
              ...
              <div className="space-y-2 text-[13px]">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-text-primary">
                    {getShiftLabel(activePolicy.shift_type)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-text-secondary">Coverage Tier</span>
                  <span className="font-medium text-text-primary">
                    {getCoverageTierLabel(activePolicy.coverage_tier)}
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-text-primary">
                    {sevConfig.multiplier}x
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-text-secondary">
                    Preview Shift Overlap
                  </span>
                  <span className="font-medium text-text-primary">
                    {formatNumber(shiftOverlap, 1)} hours
                  </span>
                </div>
                <div className="flex justify-between">
            ...
            {activeResult ? (
                ...
                <Card padding="md">
                  ...
                  <div className="space-y-2 text-[13px]">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-text-primary">
                        {getEventInfo(activeResult.event.event_type).label}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-text-secondary">Zone</span>
                      <span className="font-medium text-text-primary">
                        {activeResult.event.zone}, {activeResult.event.city}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-text-secondary">Verified</span>
                      <span className="font-medium text-text-primary">
                        {activeResult.event.verified ? "Yes" : "No"}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-text-secondary">Created</span>
                      <span className="font-medium text-text-primary">
                        {formatDateTime(activeResult.event.created_at)}
                      </span>
                    </div>
              ...
              <div className="text-center py-10 text-text-muted">
                ...
                <p className="text-[13px]">
                  Configure the disruption event and click &quot;Trigger
                  Simulation&quot; to see what happens for this active policy.
                </p>
              </div>
            )}
    ...
    return (
      <Card padding="md" className="ring-1 ring-warning">
        ...
        <p className="text-[13px] text-text-secondary">
          The live event was recorded successfully, but no claim was created for
          the active policy. This usually means the event did not overlap the
          insured shift window or fell outside the policy validity rules.
        </p>
      </Card>
    );
  }
  ...
  return (
    <Card padding="md" className="ring-1 ring-success">
      <div className="flex items-center gap-2 mb-4">
        ...
        <Badge
          variant={getClaimStatusBadgeVariant(claim.status)}
          className="ml-auto"
        >
          {claim.status}
        </Badge>
      ...
      <div className="bg-background border border-border rounded-lg p-3 mb-4 font-mono text-[12px] text-text-secondary">
        ...
        <p>
          min({formatCurrencyValue(claim.protected_hourly_income)} x{" "}
          {formatNumber(claim.affected_hours, 1)}h x {claim.severity_multiplier}
          , policy cap)
        </p>
      ...
      <div className="space-y-2 text-[13px]">
        ...
        <div className="flex justify-between">
          ...
          <span className="font-medium text-text-primary">
            {formatNumber(claim.affected_hours, 1)}h
          </span>
        </div>
        <div className="flex justify-between">
          <span className="text-text-secondary">Protected Hourly Income</span>
          <span className="font-medium text-text-primary">
            {formatCurrencyValue(claim.protected_hourly_income)}/hr
          </span>
        </div>
        <div className="flex justify-between">
          <span className="text-text-secondary">Severity Multiplier</span>
          <span className="font-medium text-text-primary">
            {claim.severity_multiplier}x
          </span>
        </div>
        <div className="flex justify-between">
          <span className="text-text-secondary">Payout Status</span>
          <Badge variant={getPayoutStatusBadgeVariant(claim.payout_status)}>
            {claim.payout_status}
          </Badge>
        </div>
        <div className="flex justify-between">
          ...
          <Badge
            variant={
              claim.anomaly_band === "HIGH"
                ? "danger"
                : claim.anomaly_band === "MEDIUM"
                  ? "warning"
                  : "success"
            }
          >
